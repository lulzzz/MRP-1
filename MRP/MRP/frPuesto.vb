Public Class frPuesto
#Region "Documentación"
    '----------------------------------------------------------------------------------------------------
    'FECHA:         04/07/2016 
    'DESARROLLADOR: Marlon Aldana
    'DESCRIPCIÓN:   Catálogo de puestos
    '----------------------------------------------------------------------------------------------------
#End Region

    Dim ControlesMaestros As New ArrayList

    Private Sub frPuesto_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ControlesMaestros.Add(tbCodigo)
        ControlesMaestros.Add(tbNombre)
        ControlesMaestros.Add(tbDescripcion)
        ControlesMaestros.Add(tbOcupacion)
        ControlesMaestros.Add(tbEmpresa)
        ControlesMaestros.Add(tbEstado)
        UcNavegador1.ControlesMaestros = ControlesMaestros
        UcNavegador1.NombreTabla = "tbl_mrp_puesto"
        UcNavegador1.QueryBuscar = "SELECT id_puesto Código, pst_nombre Nombre, pst_descripcion Descripción, pst_ocupacion Ocupacion, CASE id_estado WHEN 1 THEN 'Alta' ELSE 'Eliminado' END Estado FROM " + UcNavegador1.NombreTabla + " WHERE id_empresa=" + csDatos.IdEmpresa.ToString + " ORDER BY id_puesto ASC"
